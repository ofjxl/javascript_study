//id, pw 유효성 검사 JS
//id, pw를 입력하지 않고 로그인 클리 시 error 위치에 "정보를 입력하세요" 출력
//올바른 정보 기준 : admin / 1234
//id, pw가 모두 입력 기준으로 id가 admin이 아니면'아이디를 확인하세요' error 출력
//id, pw가 모두 입력된것 기준으로 id가 admin이 맞고 pw가 1234가 아니면 '비밀번호를 확인하세요' error 출력
//id, pw가 모두 입력 기주능로 id가 admin이고 pw도 1234면 로그인 성공 팝업 출력 
//디자인 스타일 조건 추가 
...
console.log(user_id, user_pw, error_p, login)
//id, pw를 입력하지 않고 로그인 클리 시 error 위치에 "정보를 입력하세요" 출력
//1. 클릭 이벤트 생성
let error_message = ''
login_btn.addEventListener('click',() => {
 //2. 조건문으로 id, pw 입력했는가?
    if(user_id.value == '' || user_pw.value == ''){
        ...
        /* error_p.computedStyleMap.color = 'green'
