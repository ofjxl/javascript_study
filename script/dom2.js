...
console.log(header)
//tag, class 등과 같이 다수가 존재할 수 있는 대상을 DOM으로 선택하면 특정 대상 1개만 잡히는게 아닌 collection기준으로 여러개가 잡히기 때문에 원하는 대상 사용시 정확히 그 대상을 인덱스로 선택해야한다.
...
/* 
    ...
    대신, 인덱스가 고정되므로 다른 인덱스번호에 해당하는 태그나 클래스를 사용하고 싶을 때 
    변수를 새로 만들어야한다.
    console.log(aTag2);
    console.log(aTag2[1]); //aTag[0][1] error
*/
const gnb = document.getElementById('gnb')
console.log(gnb)
// id는 단 하난로 중복할 수 없으므로 index가 들어가지 않는다. 
//float, flex, grid 
console.log('------------------')
...
/*     let total = Number(input_num.value)
    ...
    input_num.balue = Number(input_num.value)+1 
}
//객체.메서드(실행값)
//객체.속성 = 속성에 대입할 값;
function btn_func3(){
    ...
    /* window.document.write('사용자 요청사항은 입니다') */
}
function btn_fun4(){
    //사용자 요청사항과 주문수량을 확인하고
    //주문 버튼을 클릭하면 다음과 같이 경고창 출력하기 
    //"요청사항은 oooo, 주문수량은 o개, 총 결제금액은 0000원 입니다."
    //제품 1개당 1000원 기준 
    let request = input_text.value; //요청사항 저장
    let order_num = Number(input_num.value); //수량 저장
    let pay = 1000
    let total = order_num * pay
    alert(`요청사항은 ${request}, 주문수량은 ${order_num}개, 총 결제 금액은 ${total}원 입니다.`)
}